...
void dijkstras(int startNode, int endNode, std::vector<std::vector<int>> graph, std::string searchMethod) {
  ...
  if(graphConnectionControl && graph.size() > 0) {
    ...
    std::cout << "Graph is connected!" << '\n'
      << "Running Dijkstra's algortihm between node " << (char)('A' + startNode) << " as start and node " << (char)('A' + endNode) << " as end" << '\n' << '\n';

    std::cout << "Algortihm operation's: " << '\n';

    while(!priorityQueue.empty()) {
      nodeData currentNode = nodeData{priorityQueue.top().node, priorityQueue.top().weight};
      priorityQueue.pop();
      ...
      for(int i = 0; i < graph.size(); i++) {
        ...
        if(neigbourWeight != -1 && result[i].weight > currentWeight) {
          ...
          std::cout
            << "Current node " << (char)('A' + currentNode.node) << " with weight " << currentNode.weight
            << " visiting node " << (char)('A' + i) << " with weight " << graph[currentNode.node][i] << '\n';
        }
      }
    }
    ...
    for (size_t i = 0; i < result.size(); i++) {
        std::cout << (char)('A' + result[i].from) << " -> " << (char)('A' + result[i].node) << ": weight " << result[i].weight << '\n';
    }

    std::cout << '\n' << "Shortest path between node " << (char)('A' + startNode) << " and " << (char)('A' + endNode) << ":" << '\n';
...
int main(int argc, char const *argv[]) {
  ...
  // bfs(inputData(file), true);
  dijkstras(0, 5, inputData(file), "dfs");
  return 0;
}
