...
Sorting::Sorting(std::vector<int> elems) {
  number_of_elements = elems;
}

void Sorting::set_samples(int samp) {
  samples = samp;
}

void Sorting::set_data(std::vector <int> setData) {
  data = setData;
}

bool Sorting::check_sorted(std::vector<double> sorted) { // NOTE Kanske inte behövs
...
std::vector<double>  Sorting::alg_selection_sort() {
  for (int index = 0; index < number_of_elements.size(); index++) {
      ...
      std::copy(data.begin(), (data.begin() + number_of_elements[index]), data_to_sort.begin());
  		for (int i = 0; i != samples; i++) {
  			...
  			auto elapsed_microseconds_sel = std::chrono::duration_cast<std::chrono::microseconds>(end_sel - start_sel).count();
...
std::vector<double> Sorting::alg_insertion_sort() {
  for (int index = 0; index < number_of_elements.size(); index++) {
    ...
    for (int i = 0; i != samples; i++) {
      ...
      if(check_sorted(data_to_sort)) {
        std::cout << "I: " << i << '\n';
        std::cout << "Index: " << index << '\n';
      }


      auto elapsed_microseconds_ins = std::chrono::duration_cast<std::chrono::microseconds>(end_ins - start_ins).count();
  ...
  for (int index = 0; index < number_of_elements.size(); index++) {
      ...
      std::copy(data.begin(), (data.begin() + number_of_elements[index]), data_to_sort.begin());
  		...
  		for (int i = 0; i != samples; i++) {
  			...
  			auto elapsed_microseconds_quick = std::chrono::duration_cast<std::chrono::microseconds>(end_q - start_q).count();
  ...
  for (int index = 0; index < number_of_elements.size(); index++) {
      ...
      std::copy(data.begin(), (data.begin() + number_of_elements[index]), data_to_sort.begin());
  		...
  		for (int i = 0; i != samples; i++) {
  			...
  			std::chrono::microseconds elapsed_seconds_qm = std::chrono::duration_cast<std::chrono::microseconds>(end_qm - start_qm);
