...
void print_results(std::vector<double>sorting_result, int samples, std::vector<int> N, std::string type, std::string filename = "untitled.txt") {
	std::cout << "---------------------------------" << '\n';
	...
	// std::cout << "SORTING PERIODS(milliSEC):" << "\n";
	//
	// for (int i = 0; i < sorting_result.size(); i++) {
	// 	std::cout << sorting_result[i] << " ";
	// }
...
int main(){
	srand(time(NULL));
	std::vector<int> N = {2000, 4000, 8000, 10000, 11000, 12000, 14000, 16000, 18000, 20000};
	int data_size = 30000;
	int samples = 5;
	...
	int sorting_choice;
	std::cout << "Choose any of the following algorithms to sort the number container:\n";
	...
	switch (sorting_choice) {
		//Selection sort
		case 1:
			print_results(sorting.alg_selection_sort(), samples, N, "Selection sort", data_filename + "selection_sort.txt");
			break;

		//Insertion sort
		case 2:
			print_results(sorting.alg_insertion_sort(), samples, N, "Insertion sort", data_filename + "insertion_sort.txt");
			break;

		//Quick sort with partition
		case 3:
			print_results(sorting.alg_quick_partition_sort(), samples, N, "Quick sort with partition", data_filename + "quick_partition_sort.txt");
			break;

		//Quick sort with MEDIAN OF THREE.
		case 4:
			print_results(sorting.alg_quick_median_sort(), samples, N, "Quick sort with median of three", data_filename + "quick_par3.txt");
			break;

		case 5:
			print_results(sorting.alg_std_sort(), samples, N, "Std sort", data_filename + "std_sort.txt");
			break;

		case 6:
			print_results(sorting.alg_selection_sort(), samples, N, "Selection sort", data_filename + "selection_sort.txt");
			print_results(sorting.alg_insertion_sort(), samples, N, "Insertion sort", data_filename + "insertion_sort.txt");
			print_results(sorting.alg_quick_partition_sort(), samples, N, "Quick sort with partition", data_filename + "quick_partition.txt");
			print_results(sorting.alg_quick_median_sort(), samples, N, "Quick sort with median of three", data_filename + "quick_par3.txt");
			print_results(sorting.alg_std_sort(), samples, N, "Std sort", data_filename + "std_sort.txt");
